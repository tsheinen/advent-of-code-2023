...
impl Point {
    ...
    fn adjacent_points(&self) -> HashSet<Point> {
        iproduct!(-1..=1,-1..=1).filter(|(x,y)| !(*x==0&&*y==0)).map(|(x,y)| Point {x,y}).map(|x| self.clone() + x).collect()
    }
...
impl Board {
    fn find_numbers_with_adjacent_points(&self) -> HashSet<Number> {
        self.numbers.iter().cloned().filter(|num| {
            let adj_set = num.points.iter().flat_map(|p| p.adjacent_points()).collect::<HashSet<_>>();
    ...
    fn part_two(&self) -> isize {
        self.symbols.iter().cloned().filter(|sym| sym.symbol == "*").filter_map(|sym| {
            let adj_points = sym.point.adjacent_points();
            let numbers = self.numbers.iter().filter(|num| num.points.iter().any(|p| adj_points.contains(p))).collect::<Vec<_>>();
            if numbers.len() == 2 {
...
fn parse(input: &str) -> Board {
    ...
    let (numbers, symbols): (Vec<_>, Vec<_>) = input.lines().enumerate().map(|(idx, line)| {
        (number_regex.find_iter(line).map(|m| {
            Number {
                number: m.as_str().parse().expect("parse invalid int"),
                points: (m.start()..m.end()).map(|x| x as isize).map(|p| Point {y: idx as isize, x: p}).collect()
            }
        }).collect::<Vec<_>>(),
        symbol_regex.find_iter(line).map(|m| {
            Symbol {
...
fn part1(input: &Board) -> String {
    input.find_numbers_with_adjacent_points().into_iter().map(|x| x.number).sum::<isize>().to_string()
}
...
mod tests {
    ...
    #[test]
    fn points() {
        assert_eq!((Point {x:2,y:0}).adjacent_points(),[
            Point {x:1,y:-1},
            Point {x:2,y:-1},
            Point {x:3,y:-1},
            Point {x:1,y:0},
...
...$.*....
.664.598..")), "4361");
        assert_eq!(part1(&parse(include_str!("../input/2023/day3.txt").trim())), "520135");
    }
